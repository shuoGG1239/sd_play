...
class NetworkModuleLora(network.NetworkModule):
    ...
    def create_module(self, weights, key, none_ok=False):
        ...
        if is_linear:
            ...
            if weight.shape[2] != 1 or weight.shape[3] != 1:
                module = torch.nn.Conv2d(weight.shape[1], weight.shape[0], self.sd_module.kernel_size,
                                         self.sd_module.stride, self.sd_module.padding, bias=False)
            else:
    ...
    def forward(self, x, y):
        self.up_model.to(device=torch.device("cuda"))
        self.down_model.to(device=torch.device("cuda"))
