...
def get_learned_conditioning(self: sgm.models.diffusion.DiffusionEngine, batch: prompt_parser.SdConditioning | list[str]):
    ...
    devices_args = dict(device=torch.device("cuda"), dtype=torch.float16)

    sdxl_conds = {
        "txt": batch,
        "original_size_as_tuple": torch.tensor([height, width], **devices_args).repeat(len(batch), 1),
        "crop_coords_top_left": torch.tensor([0.0, 0.0], **devices_args).repeat(len(batch), 1),
...
def extend_sdxl(model):
    ...
    model.alphas_cumprod = torch.asarray(discretization.alphas_cumprod, device=torch.device("cuda"), dtype=torch.float32)

    model.conditioner.wrapped = torch.nn.Module()


sgm.modules.attention.print = print
sgm.modules.diffusionmodules.model.print = print
sgm.modules.diffusionmodules.openaimodel.print = print
sgm.modules.encoders.modules.print = print
